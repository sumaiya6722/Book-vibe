...
const Bookdetails = () => {
  ...
  const expectedBook = books.find((book) => book.bookId == id);//  gives the bookdetails of the given id
        ...
        <div className="card-body space-y-4">
          ...
          <div className="space-y-7 border-b border-gray-300 pb-5">
             ...
             <div className="flex items-center gap-3 mb-3">
               <span className="text-lg font-bold">Tags </span> {expectedBook.tags.map((tag, ind) => <div key={ind} className="badge badge-soft badge-success font-semibold">#{tag}</div>)}
             </div>   
          </div>
          ...
          <div className="flex gap-4">
            ...
            <button className="btn btn-active btn-info text-white" onClick={() => handleWishList(expectedBook)}>Add to Wishlist
            </button>
          </div>
        </div>
      </div>
